...
// export default function DashboardCards({ stats, role }: Props) {
//   const cards = [
//     {
//       title: "Companies",
      ...
      {cards.map((card) => {
        ...
        return (
          <Link
            key={card.title}
            href={card.href}
            className="group relative cursor-pointer overflow-hidden rounded-3xl border border-slate-800 bg-zinc-900/70 p-7 transition-all duration-300 hover:-translate-y-2 hover:border-cyan-500/40 hover:shadow-2xl hover:shadow-cyan-500/20 active:scale-[0.98]"
          >
            {/* Top Gradient */}
            <div
              className={`absolute inset-x-0 top-0 h-1 bg-gradient-to-r ${card.color}`}
